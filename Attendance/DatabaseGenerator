...
def insertOrUpdate(face_id,face_name,face_usn):
    ...
    cmd = 'SELECT * FROM RegisteredUsers WHERE ID="{}"'.format(face_id)
    cursor = conn.execute(cmd)
    isRecordExist = 0
    for row in cursor:
        isRecordExist = 1
    if(isRecordExist == 1):
          cmd='UPDATE RegisteredUsers SET Name = "{}", USN = "{}" WHERE ID="{}"'.format(face_name, face_usn,face_id)
    else:
        #cmd="INSERT INTO RegisteredUsers(ID,Name,USN) VALUES("+str(face_id)+","+str(face_name)+","+str(face_usn)+")"
		#cmd='INSERT INTO RegisteredUsers(ID,Name,USN) VALUES("+str(face_id)+","+str(face_name)+","+str(face_usn)+")'
		cmd='INSERT INTO RegisteredUsers VALUES("{}", "{}", "{}")'.format(face_id, face_name, face_usn)
    conn.execute(cmd)
    conn.commit()
    conn.close()
